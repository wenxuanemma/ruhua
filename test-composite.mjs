// test-composite.mjs
// Tests the Sharp compositing locally without Replicate.
// Usage:
//   node test-composite.mjs <face-image> <painting-image> <paintingId> <figureId>
//
// Example:
//   node test-composite.mjs selfie.jpg hanxizai.jpg hanxizai host
//
// Output: composited.jpg + profile.jpg in current directory
...
const [,, faceFile, paintingFile, paintingId = 'hanxizai', figureId = 'host'] = process.argv;

if (!faceFile || !paintingFile) {
  console.log('Usage: node test-composite.mjs <face.jpg> <painting.jpg> [paintingId] [figureId]');
  console.log('Example: node test-composite.mjs selfie.jpg painting.jpg hanxizai host');
  process.exit(1);
}

const region = FACE_REGIONS[paintingId]?.[figureId];
if (!region) {
  console.error(`No region for ${paintingId}/${figureId}`);
  process.exit(1);
}

async function run() {
  console.log(`\nCompositing ${faceFile} → ${paintingFile}`);
  console.log(`Painting: ${paintingId} / Figure: ${figureId}`);
  console.log(`Region: x=${region.x} y=${region.y} w=${region.w} h=${region.h} angle=${region.angle}°\n`);

  const paintingBuf = fs.readFileSync(paintingFile);
  const faceBuf     = fs.readFileSync(faceFile);

  const { width: PW, height: PH } = await sharp(paintingBuf).metadata();
  const { width: FW, height: FH } = await sharp(faceBuf).metadata();

  console.log(`Painting: ${PW}×${PH}px`);
  console.log(`Face:     ${FW}×${FH}px`);
  ...
  console.log(`Target region: ${targetX},${targetY} ${targetW}×${targetH}px\n`);

  // Crop center of face image
  ...
  // Desaturate to match aged silk palette
  const desaturatedFace = await sharp(facePng)
    .modulate({ saturation: 0.55 })
    .toBuffer();

  // Sample painting color in face region
  ...
  console.log(`Painting avg color in region: rgb(${Math.round(pR)}, ${Math.round(pG)}, ${Math.round(pB)})`);
  ...
  console.log(`Face avg color:              rgb(${Math.round(fR/fp)}, ${Math.round(fG/fp)}, ${Math.round(fB/fp)})`);

  const t = 0.55;
  const tR = Math.round((pR - fR/fp) * t);
  const tG = Math.round((pG - fG/fp) * t);
  const tB = Math.round((pB - fB/fp) * t);
  console.log(`Color tint applied:          rgb(${128+tR}, ${128+tG}, ${128+tB})\n`);
  ...
  fs.writeFileSync('composited.jpg', composited);
  console.log('✓ Saved: composited.jpg');
  ...
  fs.writeFileSync('profile.jpg', profileCrop);
  console.log('✓ Saved: profile.jpg');
  console.log('\nOpen both files to check blending quality.');
  console.log('Tweak FACE_REGIONS if position is off.');
}
