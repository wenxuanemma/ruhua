...
//
// NO restorePurchases() HERE, DELIBERATELY: Apple rejected v1.2's first
// submission (Guideline 3.1.1) because the app called StoreKit's native
// restore mechanism (Purchases.restorePurchases(), which prompts for the
// user's Apple Account password) for CONSUMABLE products -- Apple's own
// review notes state this product type cannot be restored that way, and
// asked for "your own restore mechanism" instead. That already exists:
// the Keychain-based credit migration system (lib/deviceStorage.js,
// pages/api/migrate-credits.js) runs automatically on launch, no user-
// facing restore action needed. Real device testing before this
// rejection had already confirmed the native restore call didn't
// actually recover lost consumable balances anyway (RevenueCat doesn't
// transfer Virtual Currency balances via standard restore) -- so this
// isn't a loss of working functionality, just removing a call that
// never did what it implied and that Apple correctly flagged as
// inappropriate for this product type.
//
...
export async function getCreditsBalance() {
  ...
  try {
    ...
    // CONFIRMED shape from real device logs (SDK 5.85.0, iOS):
    // { all: { CREDITS: { balance, ... } } }. Deliberately NOT hedging
    // with a speculative alternate shape here anymore — an earlier
    // version of this function silently fell back through an unconfirmed
    // flat-shape guess straight to 0, which is exactly how "balance
    // always shows 0" went unexplained for a while. If this shape is
    // ever different (future SDK version, another platform), better to
    // warn loudly here than silently return a plausible-looking wrong
    // answer again.
    const entry = virtualCurrencies?.all?.[CREDITS_CURRENCY_CODE];
    if (entry?.balance == null) {
      console.warn('[purchases] getCreditsBalance: unexpected response shape, no all.' + CREDITS_CURRENCY_CODE + '.balance found:', virtualCurrencies);
      return 0;
    }
...
// Exposed separately from purchase so callers (e.g. app launch) can
// force a fresh balance read without needing a purchase event to trigger
// it. Useful defensively: if a purchase completed during an earlier
// broken session (before some now-fixed bug), the internal invalidate
// call at the end of purchaseCredits() may never have actually run for
// that transaction, leaving the device's local SDK cache stuck on a
// stale value indefinitely since nothing else proactively refreshes it.
...
// Requests the one-time free-credit grant for this appUserID from the
// server (see pages/api/grant-free-credits.js). The actual "only once"
// guarantee is enforced server-side via a deterministic RevenueCat
// idempotency key, not by this function or its caller — this is just the
// client-side trigger, safe to call more than once for the same user
// without risk of a double grant.
export async function grantFreeCredits() {
  const appUserID = await getAppUserID();
  if (!appUserID) return false;
  try {
    const res = await fetch('/api/grant-free-credits', {
      method: 'POST',
    ...
    console.warn('[purchases] grantFreeCredits failed:', e);
    return false;
  }
}
